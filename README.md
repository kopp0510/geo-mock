...
- **已存地點**（popup）→ 按「＋ 存目前座標」給它一個名字，之後點 chip 就套用回來，
  按 × 刪掉。上限 12 個
- **語言**（popup 底部）→ 繁體中文／English／跟隨瀏覽器，選了立刻換，不必重開
...
第三版做了：
- `watchPosition` / `clearWatch` —— 固定模式送一次就安靜，抖動模式每秒送一組新座標，
  開關關掉時既有的 watch 交回原生
...
**還沒做**：per-site 白名單（現在是全有全無，開了就對所有網站生效）。
...
```bash
node tools/verify.js        # exit 0 = 十三項斷言全過
```
