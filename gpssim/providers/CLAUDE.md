...
- **不要拿「問不到版本」當作不支援**。版本只是報告上的字串，能不能跑取決於
  執行檔在不在。踩過：Windows 的 `chrome.exe --version` 印不出東西，
  `is_supported()` 因此回 False，整個拒跑，而 Chrome 好好地裝在那裡
...
`cdp.CDP` 內部有一把 `RLock`，`send()` 與 `pump()` 全程持有。
**這不是防呆，是必要的**：`send()` 的語意是「寫出去、一路讀到自己的 id 為止」，
兩個執行緒交錯時 A 會讀走 B 在等的回覆再丟掉，B 就永遠等不到。
路線播放正是這個情境 —— player 執行緒每秒送位置，驗證那端同時在讀軌跡。

代價是兩邊會互相等，但每個 CDP 呼叫都在毫秒等級，不影響路線的節奏
（`player` 是對「出發時間」重算每一拍該送的時刻，不是睡固定秒數）。
