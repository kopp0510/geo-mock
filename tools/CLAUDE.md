...
6. **改設定不重整分頁也生效**（options 存第二組座標 → **不** `page.goto` →
   ...
   **注意這一項只驗座標路徑**：「切換開關也即時生效」目前只有手動驗證涵蓋
   （第 9 項雖然測開關，但它刻意重新載入頁面，靠 `announced` 重置取得乾淨訊號）
...
8. **推送的內容不含已存地點**（在測試頁監聽 `geo-mock:settings`，斷言 detail 裡
   ...
   沒有任何症狀**，只有這一項擋得住那種回退
9. 關掉開關後不再覆寫（經 popup 取消勾選 → 重新載入測試頁）。第 2～8 項全在測
   「開啟」狀態，`enabled: false` 這條路徑只有這一項驗得到。斷言看的是
   `inject.js` 有沒有印出覆寫痕跡，不是比對座標數值 —— 只比座標的話，
   「停用分支誤送預設值」這種迴歸會綠燈放行
10. 設定永不到達時仍會回應（腳本會即時造一個「只有 inject.js、沒有 bridge.js」的
...
第 4～9 項需要 extension id，靠讀 `chrome://extensions` 的 shadow DOM 取得
