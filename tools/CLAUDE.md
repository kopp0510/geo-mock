...
10. **推送的內容不含已存地點**（在測試頁監聽 `geo-mock:settings`，斷言 detail 裡
...
12. 關掉開關後不再覆寫（經 popup 取消勾選 → 重新載入測試頁）。**等的是
   ...
   「停用分支誤送預設值」這種迴歸會綠燈放行
13. 設定永不到達時仍會回應（腳本會即時造一個「只有 inject.js、沒有 bridge.js」的
