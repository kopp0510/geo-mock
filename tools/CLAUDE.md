...
10. **推送的內容不含已存地點與排除清單**（在測試頁監聽 `geo-mock:settings`，斷言 detail 裡
   沒有 `places`、也沒有地點名稱字串；同時斷言存地點這個寫入觸發 0 次推送）。
   這個事件頁面自己的 JS 監聽得到，少了 `bridge.js` 的 `pick()` 過濾，使用者
   自己命名的地點簿連同精確座標會被每個網站讀走 —— **而且功能完全正常、
   沒有任何症狀**，只有這一項擋得住那種回退
11. **`watchPosition` / `clearWatch`**：id 同步回傳（是 `number`，不是 undefined）、
   固定模式只送一次、切到 jitter 後持續送且座標互不相同、`clearWatch` 之後完全安靜。
   這一項約 7 秒，是全部斷言裡最慢的，因為要真的等計時器跑
12. **排除清單上的網站走真實定位（連頁內的跨來源 frame），拿掉後恢復**。
    全程不重整測試頁（清單變更也走 storage.onChanged 那條推送路徑）。
    **三件事一起驗**：只驗「排除後不覆寫」的話，一個永遠回真實定位的迴歸也會
    綠燈；只驗「拿掉後恢復」則根本沒測到排除本身；而第三件是這一項的重點 ——
    測試中會即時塞一個 `http://127.0.0.1:PORT/frame.html` 的 iframe，
    **它自己的 host 不在清單上**（清單寫的是 `localhost:PORT`），所以只有
    `bridge.js` 的 `topHost()`（最上層文件也算排除）能讓它停下來。
    沒有這一段的話那個函式從頭到尾不會被執行到，而它正是「排除的是網站不是
    frame」那個修正的全部內容 —— 少了它，被排除的網銀頁面裡嵌的地圖 iframe
    仍在回報假座標
13. 關掉開關後不再覆寫（經 popup 取消勾選 → 重新載入測試頁）。**等的是
   ...
   「停用分支誤送預設值」這種迴歸會綠燈放行
14. 設定永不到達時仍會回應（腳本會即時造一個「只有 inject.js、沒有 bridge.js」的
